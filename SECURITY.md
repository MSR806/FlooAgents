...
Floo Agents is in active development with no stable release. Only `master` is supported —
fixes land there and are not backported.

## What this project handles

Floo Agents is designed to hold credentials, so a few areas deserve extra care when
reporting or reviewing:

- **The gateway vault** (`apps/gateway/src/vault.ts`) — provider credentials encrypted
  at rest with `CREDENTIAL_VAULT_KEY`. Credentials must never appear in tool output, run
...
- **Composio** — only its project API key belongs in the platform vault/environment. Downstream provider
...
## Running Floo Agents yourself

Floo Agents executes model-authored code and shell commands by design. Self-hosted
...
The web management UI currently has no browser authentication. The internal `INTERNAL_API_TOKEN`
is shared only by the control plane and gateway and must never reach the browser. Docker binds
published ports to loopback by default; set `BIND_ADDRESS` only to an address on a trusted
