...
- **Agent tools** are high-level Gilly abstractions — `Read` / `Write` / `Bash` — stored in config. The harness (`apps/harness/src/harness-claude/loop.ts` `expandTools`) maps them to concrete SDK tools (e.g. `Read → Read/Glob/Grep`). **Never surface SDK tool names above the harness**; the UI/DB/API see only the abstractions.
