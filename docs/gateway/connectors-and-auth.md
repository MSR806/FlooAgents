# Floo Agents — Gateway Connectors & Auth
...
- **API** — REST/GraphQL services we write tools for (`defineTool` handlers): Branch, Meta, and
  the internal agent-management API.
...
Composio Connect Link for the shared deployment identity, and Composio owns the downstream tokens.
