...
2. **Script** — chained or heavy calls: the agent writes a TypeScript file in its workspace and runs it with Bash. The script imports `@floo/gateway-client` (authenticated via `TOOL_GATEWAY_URL` / `TOOL_GATEWAY_TOKEN` env), chains as many tool calls as it needs, and prints only the summary. Raw payloads live and die inside the sandbox.
