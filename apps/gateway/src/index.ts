...
const GATEWAY_URL = process.env.TOOL_GATEWAY_URL ?? `http://localhost:${PORT}`;
...
if (!vaultKey) throw new Error("CREDENTIAL_VAULT_KEY is required");
const adminToken = process.env.INTERNAL_API_TOKEN;
if (!adminToken) throw new Error("INTERNAL_API_TOKEN is required");
...
console.log(`⚡️ Tool Gateway listening on :${PORT}`);
